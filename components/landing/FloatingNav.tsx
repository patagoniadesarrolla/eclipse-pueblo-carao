...
const SECTIONS = [
  { id: 'hero',      label: 'Inicio',          symbol: '🌒' },
  { id: 'countdown', label: 'Cuenta regresiva', symbol: '◎' },
  { id: 'programa',  label: 'Programa',         symbol: '◈' },
  { id: 'includes',  label: '¿Qué incluye?',    symbol: '✦' },
  { id: 'location',  label: 'El lugar',         symbol: '◉' },
  { id: 'faq',       label: 'FAQ',              symbol: '?' },
]
...
export default function FloatingNav() {
  ...
  useEffect(() => {
    // Mostrar nav después de bajar un poco
  ...
  return (
    ...
    >
          ...
          <div key={section.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: '50%',
                border: isActive ? '2px solid #7c3aed' : '1.5px solid rgba(255,255,255,0.25)',
            ...
            {i < SECTIONS.length - 1 && (
              <div style={{
                width: '1px',
                height: '18px',
                background: active === section.id || active === SECTIONS[i + 1].id
      ...
      <style>{`
        @keyframes navPulse {
          0%, 100% {
            box-shadow: 0 0 0 4px rgba(124,58,237,0.2), 0 0 12px rgba(124,58,237,0.6);
          }
          50% {
            box-shadow: 0 0 0 7px rgba(124,58,237,0.1), 0 0 20px rgba(124,58,237,0.8);
          }
        }
      `}</style>
    </div>
